...
return a.$apply(function(){if(b.shiftKey)return a.multiSelect.includeUntil(c);if(b.metaKey)return c.selected?a.multiSelect.exclude(c,!1):a.multiSelect.include(c,!1)})})}}};g.$inject=["KeymapUtils","MultiSelect"];angular.module("akop-multi-select").directive("akopMultiSelect",g);angular.module("akop-query-filter",[]);g=function(d,b,a){var c;c=function(a,b,f){var k,e;for(null==f&&(f=[]);a.length;)k=d("filter")(b,a.pop(),!0),f.push(k),c(a,(e=_(b)).without.apply(e,k),f);return _(f).flatten()};return function(d,
h){var f,k,e;h=b(h);e=h.group_by;delete h.group_by;f=a.getArrayArguments(h);k=c(a.arrayArgsToExpressions(h),d);f=0<f.length?c([h],k):c([h],d);if(!e)return f;if(_.any(d,function(a){return void 0===a[e]}))throw Error('serviceQueryFilter: Missing group_by property "'+e+'" for some or all elements in list.');return _.groupBy(f,e)}};g.$inject=["$filter","akopParser","expressionUtils"];angular.module("akop-query-filter").filter("akopQuery",g);angular.module("akop-query-filter").factory("akopParser",function(){return function(d){var b,
a,c,l,h,f,k,e,g;if(d===Object(d)||!(1<d.split(":").length))return d;b=/(\w+):((\w+)|(\[.*\])|"(.*)")/g;for(c=[];a=b.exec(d);)c.push(a);l=function(a){return String(a).replace(/^[\s\"]+|[\s\"]+$/g,"")};d={};e=0;for(g=c.length;e<g;e++)a=c[e],b=a[1],a=a[5]||a[2],(f=a.match(/\[(.*)\]/))?(k=f[1].split(","),d[b]=function(){var a,b,c;c=[];a=0;for(b=k.length;a<b;a++)h=k[a],c.push(""+l(h));return c}()):d[b]=""+a;return d}});g=function(){function d(){}d.prototype.getArrayArguments=function(b){var a,c;return _.compact(function(){var d;
