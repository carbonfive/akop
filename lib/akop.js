...
(function() {
  ...
  MultiSelect = (function() {
    ...
    MultiSelect.prototype.toggle = function(el) {
      if (el.selected) {
        return this.exclude(el, false);
      } else {
        return this.include(el, false);
      }
    };
  ...
  multiSelectDirective = function(KeymapUtils, MultiSelect, $parse) {
    return {
      ...
      link: function($scope, el, attrs) {
        var self, targetScope;
        self = $scope.$new();
        targetScope = function(event) {
          return angular.element(event.target).scope();
        };
        self.getSelection = function(event) {
          return $parse(attrs['selectableName'])(targetScope(event));
        };
        self.keymap = {
          'up': function() {
        ...
        $scope.multiSelectClickHandler = function(event, item) {
          if (event.shiftKey) {
            return $scope.multiSelect.includeUntil(item);
          } else if (event.metaKey) {
            return $scope.multiSelect.toggle(item);
          } else {
        ...
        return el.bind('click', function(e) {
          return $scope.$apply(function() {
            return $scope.multiSelectClickHandler(e, self.getSelection(e));
          });
        });
      }
    };
