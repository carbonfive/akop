...
(function() {
  ...
  describe('MultiSelect', function() {
    ...
    describe('constructor', function() {
      it('assigns root and list', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[2];
        multi = new MultiSelect(list, root);
        expect(multi.root).toEqual(root);
        return expect(multi.list).toEqual(list);
      }));
      it('throws an error if root is not a member of list', inject(function(MultiSelect) {
        return expect((function(_this) {
          return function() {
            return new MultiSelect(buildList(), {
              name: 'one hundred'
            });
          };
        })(this)).toThrow(new Error('MultiSelect: Element must be a member of list.'));
      }));
      it('sets the cursor to the position of root', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[2];
        multi = new MultiSelect(list, root);
        return expect(multi.cursor).toEqual(list.indexOf(root));
      }));
      it('accepts a list with no root', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
      ...
      return describe('when initialized with elements that have a selected property', function() {
        return it('adds selects the elements', inject(function(MultiSelect) {
          var list, multi;
          list = buildList();
          list[1].selected = true;
          multi = new MultiSelect(list);
          return expect(multi.selected).toEqual([void 0, list[1]]);
        }));
      });
    });
    describe('include', function() {
      beforeEach(inject(function(MultiSelect) {
        this.list = buildList();
        this.root = this.list[2];
        this.multi = new MultiSelect(this.list, this.root);
    ...
    describe('includeUntil', function() {
      ...
      return describe('given a list without selected items', function() {
        beforeEach(inject(function(MultiSelect) {
          this.list = buildList();
          this.multi = new MultiSelect(this.list);
          this.multi.reset();
    ...
    describe('exclude', function() {
      ...
      return describe('for non-adjacent elements', function() {
        beforeEach(inject(function(MultiSelect) {
          this.list = buildList();
          this.multi = new MultiSelect(this.list, this.list[1]);
          this.multi.include(this.list[4], false);
          return this.selected = this.multi.exclude(this.list[4], false);
        }));
        it('sets the selected property of the passed element to false', function() {
          return expect(this.item4.selected).toBe(false);
        });
        it('removes the passed element from selected', function() {
          return expect(this.multi.selected.indexOf(this.list[4])).toEqual(-1);
        });
        return it('sets the cursor to the next closest selected element', function() {
          return expect(this.multi.cursor).toEqual(this.selected.indexOf(this.list[1]));
        });
      });
    });
    describe('reset with item', function() {
      ...
      return it('sets the selected property of all other elements to false', function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          element = _ref[_i];
          if (element !== this.item4) {
            _results.push(expect(element.selected).toBeFalsy());
          } else {
    ...
    describe('reset without item', function() {
      beforeEach(inject(function(MultiSelect) {
        this.list = buildList();
        this.root = this.list[2];
        return this.multi = new MultiSelect(this.list, this.root);
      }));
    ...
    describe('selectPrev', function() {
      it('selects the last element if no element is selected', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
        multi = new MultiSelect(list, this.root);
        return expect(multi.selectPrev()).toSelect(list, 6);
      }));
      it('selects the previous adjacent element', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[2];
        multi = new MultiSelect(list, root);
      ...
      return it('selects the last element if the current element is the first', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[0];
        multi = new MultiSelect(list, root);
    ...
    describe('selectNext', function() {
      it('selects the first element if no element is selected', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
        multi = new MultiSelect(list);
        return expect(multi.selectNext()).toSelect(list, 0);
      }));
      it('selects the next adjacent element', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[2];
        multi = new MultiSelect(list, root);
        return expect(multi.selectNext()).toSelect(list, 3);
      }));
      return it('selects the first element if the current element is last', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = _.last(list);
        multi = new MultiSelect(list, root);
    ...
    describe('moveToPrev', function() {
      it('expands the selection to include the prev adjacent element', inject(function(MultiSelect) {
        var list, multi, root;
        list = buildList();
        root = list[2];
        multi = new MultiSelect(list, root);
        return expect(multi.moveToPrev()).toSelect(list, 1, 2);
      }));
      return it('does not wrap around to the top of the list', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
        multi = new MultiSelect(list, list[0]);
        return expect(multi.moveToPrev()).toSelect(list, 0);
      }));
    });
    return describe('moveToNext', function() {
      it('expands the selection to include the next adjacent element', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
        multi = new MultiSelect(list, list[2]);
        multi.moveToNext();
        return expect(multi.selected).toSelect(list, 2, 3);
      }));
      return it('does not wrap around to the bottom of the list', inject(function(MultiSelect) {
        var list, multi;
        list = buildList();
        multi = new MultiSelect(list, list[6]);
        multi.moveToNext();
        return expect(multi.selected).toSelect(list, 6);
      }));
    });
  });
